...
class DepartamentoDataTable extends DataTable implements FromView, ShouldAutoSize, WithEvents, WithTitle
{
    ...
    public function __construct()
    {
        $this->defaultProperties = collect([
            'namePluralModel' => 'Departamentos',
            'nameSingularModel' => 'Departamento',
            'routeNew' => route('departamento.create')
        ]);
    }
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($departamento) {
                return
                    '<a role="button" href="'.route('departamento.edit', [$departamento->id]).'" class="btn btn-sm btn-outline-info" data-toggle="tooltip" data-animation="true" data-placement="top" title="Editar"><i class="fas fa-user-edit"></i></a>
                    <a role="button" href="'.route('departamento.show', [$departamento->id]).'" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-animation="true" data-placement="top" title="Ver"><i class="fas fa-eye"></i></a>';
            })
            ->addColumn('estado', function ($departamento) {
                return "<a class='badge btn-change-status ".(($departamento->estado == 'Activo') ? "badge-success" : "badge-warning")."' data-item-id='".$departamento->id."' data-item-name='".$departamento->nombre."' data-item-status='".$departamento->estado."' href='#'>$departamento->estado</a>";
            })
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('region')->addClass('text-break'),
